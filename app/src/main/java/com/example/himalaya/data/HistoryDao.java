    ...
    public void addHistory(Track track) {
        synchronized (mLock) {
            ...
            try {
                ...
                //先去删除
                int delResult = db.delete(Constants.HISTORY_DB_NAME, Constants.HISTORY_TRACK_ID + "=?", new String[]{track.getDataId() + ""});
                ...
                //插入数据
                db.insert(Constants.HISTORY_DB_NAME, null, values);
    ...
    public void delHistory(Track track) {
        synchronized (mLock) {
            ...
            try {
                db = mDbHelper.getWritableDatabase();
                db.beginTransaction();
                int delete = db.delete(Constants.HISTORY_DB_NAME, Constants.HISTORY_TRACK_ID + "=?", new String[]{track.getDataId() + ""});
    ...
    public void clearHistory() {
        synchronized(mLock) {
            ...
            try {
                db = mDbHelper.getWritableDatabase();
                db.beginTransaction();
                db.delete(Constants.HISTORY_DB_NAME,null,null);
                db.setTransactionSuccessful();
                isDeleteSuccess = true;
            } catch(Exception e) {
                e.printStackTrace();
    ...
    public void listHistory() {
        synchronized(mLock) {
            ...
            try {
                ...
                db.beginTransaction();
                Cursor cursor = db.query(Constants.HISTORY_DB_NAME,null,null,null,null,null,"_id desc");
